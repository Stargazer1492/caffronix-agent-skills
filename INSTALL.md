...
| `campus-job-hunt` | `campus-job-hunt/` | 面向校招求职的个人执行工作流，覆盖简历和JD附件解析、公司岗位调研、JD深度分析、简历诊断、HTML报告与3:4竖版2K图片生成、投递管理、面试复盘和offer比较。 |
...
2. 使用 Codex 官方 `$skill-installer` 从 GitHub 仓库安装指定目录：
   ...
   对 `campus-job-hunt`，安装目标参数应为：

   ```text
   repo: Stargazer1492/caffronix-agent-skills
   path: campus-job-hunt
   ref: main
   ```
