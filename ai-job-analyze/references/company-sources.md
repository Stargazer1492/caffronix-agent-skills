...
- 优先使用当前 agent 的浏览器能力访问公开页面并观察页面行为；脚本 adapter 是 fallback，不是唯一抓取路径。
- 每个适配器必须以“公司 + 渠道”命名。
- 校招和社招不得共用适配器。
- 适配器负责发现公开岗位详情页和保留原始字段，不负责最终人工智能岗位语义判断。
- 人工智能岗位判断放在分析层，避免某家公司筛选参数变化导致口径漂移。
- 适配器不只需要入口 URL，还必须知道查询词如何传入、页码如何推进、总量上限如何生效。
- `scripts/config.toml` 中的 `sources.*` 是默认来源契约；用户命令行提供 source URL 时可以覆盖默认 URL，但不能跳过 adapter 对分页和详情页的处理。
