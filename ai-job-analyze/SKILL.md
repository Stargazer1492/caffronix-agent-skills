---
name: ai-job-analyze
description: 抓取字节、阿里、腾讯、美团公开招聘站点中的人工智能岗位，分析校招或社招机会，并生成本地优先的网页或图片洞察报告。适用于用户要求收集人工智能岗位、比较校招/社招渠道、或基于公开招聘页生成可视化岗位市场报告的场景。
---
...
5. 真正抓取前先收集用户意图。用户没有给出时，读取 `config.toml` 中的默认值：
...
6. 如果用户只给出跨公司分析需求，没有明确公司、渠道或来源，先按 `references/task-decomposition-sop.md` 拆解任务，生成本次抓取计划。计划必须遵守 `config.toml` 中的 `crawl.max_jobs_per_task`。

7. 按阶段执行，不把抓取、字段归一、洞察和报告导出塞进一个长任务：
   - 抓取阶段：运行 `python scripts/run.py crawl`，输出原始岗位与失败项。
...
- 不要求 `.env.example`。默认值放在可 Git 管理的 `config.toml` 中，且只允许非敏感配置。
